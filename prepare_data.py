...
from preparation import aihub_mmv, iemocap, selvas, man2, girl2, one
...
def main(args, preprocess_config):
    ...
    elif 'selvas' in preprocess_config['dataset']:
        if args.extract_lexicon:
            selvas.extract_lexicon(preprocess_config)
        else:
            selvas.create_dataset(preprocess_config)

    elif "man2" in preprocess_config['dataset']:
        if args.extract_lexicon:
            man2.extract_lexicon(preprocess_config)
        else:
            man2.create_dataset(preprocess_config)

    elif "girl2" in preprocess_config['dataset']:
        if args.extract_lexicon:
            girl2.extract_lexicon(preprocess_config)
        else:
            girl2.create_dataset(preprocess_config)

    elif "one" in preprocess_config['dataset']:
        if args.extract_lexicon:
            one.extract_lexicon(preprocess_config)
        else:
            one.create_dataset(preprocess_config)
