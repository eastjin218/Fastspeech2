...
def synth_samples2(targets, predictions, vocoder, model_config, preprocess_config, path, tag=None):
    ...
    for i in range(len(predictions[0])):
        ...
        plt.savefig(os.path.join(path, "{}.png".format(f"_{tag}" if tag is not None else "")))
        plt.close()
    ...
    for wav, basename in zip(wav_predictions, basenames):
        wavfile.write(os.path.join(path, "{}.wav".format( f"{tag}" if tag is not None else "")), sampling_rate, wav)
...
#    import time
#    now = time.localtime()
#    time = "%04d_%02d_%02d-%02d_%02d_%02d" % (now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec)
    output = '/home/ubuntu/ldj/mission1/audio'
    sampling_rate = preprocess_config["preprocessing"]["audio"]["sampling_rate"]
    for wav, basename in zip(wav_predictions, basenames):
        #wavfile.write(os.path.join(path, f"{time}.wav"), sampling_rate, wav)
        wavfile.write(os.path.join(output, f"{basename}.wav"), sampling_rate, wav)
    return basename 
